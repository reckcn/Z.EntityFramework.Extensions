...
namespace Z.EntityFramework.Extensions.Demos.SqlServer
{
    ...
    {
        public static void RetryCount(int nbRecords, StringBuilder sb)
        {
            using (var ctx = new CodeFirstEntities())
            {
                ...
                {
                    // RetryCount property allows to retry (open the connection or execute the command) X times when a transient error occurs
